...
let type_of_type_expression strict_flag typexp =
  ...
  | Ztypeconstr(cstr_name, args, dimargs) ->
      let cstr =
        try
      ...
      else if list_length dimargs > cstr.info.ty_dimarity then
        dim_arity_err cstr dimargs typexp.te_loc
      else
      begin
         if (list_length dimargs < cstr.info.ty_dimarity) & (!warning) then
            implicit_dim_arg_warning cstr dimargs typexp.te_loc;
        { typ_desc = Tconstr(cstr.info.ty_constr, map type_of args, 
            (map (calcdim strict_flag) dimargs) @ 
            (opt_dim_var_list (cstr.info.ty_dimarity - list_length dimargs)) );
...
let type_of_atomic_constant = function
    ACint _ -> type_int (opt_dim_var())
  | ACfloat _ -> type_float (opt_dim_var())
  ...
  let rec scan_format i =
    ...
    match nth_char fmt i with
      `%` ->
        ...
        | `d` | `o` | `x` | `X` | `u` ->
            type_arrow (type_int (opt_dim_var ()), scan_format (succ j))
        | `f` | `e` | `E` | `g` | `G` ->
            type_arrow (type_float (opt_dim_var ()), scan_format (succ j))
        | `b` ->
...
let rec type_expr env expr =
  ...
  | Zfor (id, start, stop, up_flag, body) ->
      let ty_boundvar=type_int (opt_dim_var ()) in
      type_expect env start ty_boundvar;
      type_expect env stop ty_boundvar;
      type_statement ((id,(ty_boundvar,Notmutable)) :: env) body;
      type_unit
