...
type module =
  { mod_name: string;                        (* name of the module *)
    ...
    mod_dim: (string, string global) hashtbl__t;
                                             (* table of dimensions *)
...
and dim_of_module     md = md.mod_dim
;;
...
let open_module name =
  ...
  add_table module.mod_dim (!opened_modules).mod_dim;
...
and add_dim = add_global_info dim_of_module
;;
...
and find_dim = find_desc dim_of_module
;;
