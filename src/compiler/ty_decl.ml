...
let enter_new_abbrev (ty_constr, ty_params, dim_params, body) =
  let ty_body = type_of_type_expression true body in
    pop_type_level();
    generalize_type ty_body;
    do_list generalize_type ty_params;
    do_list generalize_dim dim_params;
    ty_constr.info.ty_abbr <- Tabbrev(ty_params, dim_params, ty_body);
    Abbrev_type(ty_params, dim_params, ty_body)
;;

let enter_new_type (ty_name, params, dimparams, def) =
  let ty_constr =
  ...
  let ty_desc =
    defined_global ty_name
      { ty_constr = ty_constr;
        ...
        ty_dimarity = list_length dimparams;
        ty_desc  = Abstract_type } in
  add_type ty_desc;
  (ty_desc, params, dimparams, def)
;;
...
let define_new_type loc (ty_desc, params, dimparams, def) =
  push_type_level();
  let ty_params =
    try
  ...
  let ty_dimparams =
    try
      bind_dim_expression_vars dimparams
    with Failure "bind_dim_expression_vars"->
      duplicate_param_in_type_decl_err loc in
  let ty_res =
    { typ_desc = Tconstr(ty_desc.info.ty_constr, ty_params, 
        map (fun v -> [(v,1,1)] ) ty_dimparams);
  ...
  let type_comp =
    ...
    | Zabbrev_type body ->
        enter_new_abbrev (ty_desc.info.ty_constr, ty_params, 
          map (fun v -> [(v,1,1)] ) ty_dimparams, body) in
  ...
  begin try
    ...
    if extdef.et_descr.info.ty_dimarity <> ty_desc.info.ty_dimarity then
      dim_decl_arity_err loc extdef.et_descr ty_desc;
...
let check_recursive_abbrev loc (ty, params, dimparams, def) =
  try
