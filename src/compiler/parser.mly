...
Simple_type :
        Type_var
          { make_typ(Ztypevar $1) }
      | Opt_type_par Opt_dim_par Ext_ident
          { make_typ(Ztypeconstr($3, $1, $2)) }
      | LPAREN Type RPAREN
          { $2 }
;
...
Opt_dim_par :
       LBRACKET Dim_comma_list RBRACKET
       { $2 }
     | 
       { [] }
;
...
Dim_params :
      LBRACKET Type_var_list RBRACKET
       { $2 }
     |
       { [] }
;
...
Opt_type_par :
     Simple_type
      { [$1] }
   | LPAREN Type COMMA Type_comma_list RPAREN
      { $2::$4 }
   | 
      { [] }
;
...
Type1_decl :
        Type_params Dim_params IDENT Type1_def
          { ($3, $1, $2, $4) }
;
