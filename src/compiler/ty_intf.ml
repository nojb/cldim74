...
let enter_interface_definitions intf =
  ...
  hashtbl__do_table
    (fun name d_desc -> let _ = define_new_dim d_desc in ())
    (dim_of_module intf);
  hashtbl__do_table
    (fun name val_desc ->
...
let check_value_match val_decl =
  ...
  let (nongen_vars,nongendim_vars) = 
           free_type_vars notgeneric val_impl.info.val_typ in
  begin try
    filter (type_instance val_impl.info.val_typ, val_decl.info.val_typ)
  with Unify ->
    type_mismatch_err val_decl val_impl
  end;
  if (exists (fun ty -> free_type_vars generic ty <> ([],[])) nongen_vars)
  or (exists (fun dim -> free_dim_vars generic dim != [])
    (map (fun v -> [(v,1,1)]) nongendim_vars)) then
