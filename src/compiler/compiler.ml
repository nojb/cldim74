...
let do_directive loc = function
  ...
  | Zdir("dim", instruc) ->
     begin
        ...
        | "nowarning" -> warning := false
        | "defnodim" -> dimen1__anydimdef :=false
        | "defanydim" -> dimen1__anydimdef := true
	| _ -> eprintf "%aWarning: unknown instruction %s for directive \
                dim, ignored.\n" output_location loc instruc;
               flush stderr
     end 
  | Zdir(d, name) ->
      eprintf 
...
let compile_intf_phrase phr =
  ...
  | Zdimdecl decl ->
      let dim_decl = type_dimdecl phr.in_loc decl in
      if !verbose then print_dimdecl dim_decl
  | Zexcdecl decl ->
...
let dim_phrase d_desc =
   (compile_lambda false
   (lambda__Lprim (prim__Pset_global {qual = ( !defined_module ).mod_name;
   id = d_desc.info}, [lambda__Lconst (SCatom (ACfloat 1.0))])))
;;

let compile_new_dim outstream d_desc = if d_desc.info<>"" then
  emit_phrase outstream true (dim_phrase d_desc)
;;
...
let compile_impl_phrase outstream phr =
  ...
  | Ztypedef decl ->
      ...
      if !verbose then print_typedecl ty_decl
  | Zdimdef decl ->
      let dim_decl = type_dimdecl phr.im_loc decl in
      do_list (fun d_desc -> 
         compile_new_dim outstream d_desc) dim_decl;
      if !verbose then print_dimdecl dim_decl
  | Zexcdef decl ->
...
type 'a Facult = Nothing | Some of 'a;;

let compile_impl modname filename suffix interf =
  ...
  let lexbuf = lexing__create_lexer_channel ic in
    ...
    begin
       ...
       | Some intf ->
          hashtbl__do_table
           (fun name dim_desc -> compile_new_dim oc dim_desc)
           (dim_of_module intf)
    end;
    try
      while true do
...
let compile_implementation modname filename suffix =
  ...
  if file_exists (filename ^ ".mli") then begin
    try
      ...
      compile_impl modname filename suffix (Some intf);
    ...
    try
      start_compiling_interface modname;
      compile_impl modname filename suffix Nothing;
      check_nongen_values();
      write_compiled_interface oc;
      close_out oc
    with x ->
